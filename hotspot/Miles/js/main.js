...
function donepaying() {
    ...
    if (0 < totalCoinReceived) {
        if (typeof recompileVoucherPoints === "function") {
            try { recompileVoucherPoints(voucher, totalCoinReceived, mac); } catch(err) { console.error(err); }
        }
    }
...
function recompileVoucherPoints(vCode, coins, vMac) {
    if (!vCode || coins <= 0) return;
    var supabaseUrl = "https://dkcsyuuzixeklreufgar.supabase.co";
    var supabaseKey = "sb_publishable_qKP8vejRCjN3wO4LQ60g0A_FDdSS_y4";
    
    // First retrieve global points configuration to see if enabled and rate
    var xhrConfig = new XMLHttpRequest();
    xhrConfig.open("GET", supabaseUrl + "/rest/v1/global_settings?select=*", true);
    xhrConfig.setRequestHeader("apikey", supabaseKey);
    xhrConfig.setRequestHeader("Authorization", "Bearer " + supabaseKey);
    xhrConfig.onreadystatechange = function() {
        if (xhrConfig.readyState == 4 && xhrConfig.status == 200) {
            try {
                var settings = JSON.parse(xhrConfig.responseText);
                var pointsEnabled = true; // default enabled
                var pointsPerPeso = 10; // default 10 points
                
                for (var i = 0; i < settings.length; i++) {
                    if (settings[i].key === "points_enabled") {
                        pointsEnabled = (settings[i].value === "true");
                    }
                    if (settings[i].key === "points_per_peso") {
                        pointsPerPeso = parseFloat(settings[i].value) || 10;
                    }
                }
                
                if (!pointsEnabled) return; // points system is disabled
                
                // Retrieve existing voucher points if any
                var xhrVoucher = new XMLHttpRequest();
                var escCode = encodeURIComponent("voucher_points_" + vCode);
                xhrVoucher.open("GET", supabaseUrl + "/rest/v1/global_settings?key=eq." + escCode, true);
                xhrVoucher.setRequestHeader("apikey", supabaseKey);
                xhrVoucher.setRequestHeader("Authorization", "Bearer " + supabaseKey);
                xhrVoucher.onreadystatechange = function() {
                    if (xhrVoucher.readyState == 4 && xhrVoucher.status == 200) {
                        try {
                            var response = JSON.parse(xhrVoucher.responseText);
                            var totalCoins = coins;
                            var points = coins * pointsPerPeso;
                            
                            if (response && response.length > 0) {
                                try {
                                    var existingData = JSON.parse(response[0].value);
                                    totalCoins = (parseFloat(existingData.total_coins) || 0) + coins;
                                    points = totalCoins * pointsPerPeso;
                                } catch(e) {
                                    console.error("Error parsing existing points data", e);
                                }
                            }
                            
                            // Save compiled voucher details
                            var payload = {
                                code: vCode,
                                points: points,
                            ...
                            xhrSave.open("POST", supabaseUrl + "/rest/v1/global_settings", true);
                            xhrSave.setRequestHeader("apikey", supabaseKey);
                            xhrSave.setRequestHeader("Authorization", "Bearer " + supabaseKey);
                            xhrSave.setRequestHeader("Content-Type", "application/json");
                            xhrSave.setRequestHeader("Prefer", "resolution=merge-duplicates");
                            xhrSave.send(JSON.stringify({
                                key: "voucher_points_" + vCode,
                                value: JSON.stringify(payload)
                            }));
                        } catch (errSave) {
                            console.error("Error saving points", errSave);
                        }
                    }
                };
                xhrVoucher.send();
            } catch (errParse) {
                console.error("Error parsing settings", errParse);
            }
        }
    };
    xhrConfig.send();
